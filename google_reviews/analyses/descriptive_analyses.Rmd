...
```{r}
ggmap(map) +
  geom_point(aes(x = lon, y = lat, alpha = mean), 
             data = affin.cord, 
             color = "red") +
  ggtitle("Sentiment") +
  theme_bw()
...
```{r}
correlate(all.lang[,-1]) %>%
  focus(TTR:log.prop.emoji, mirror = TRUE) %>%
  shave(upper = TRUE) %>%
  rplot()
```
...
```{r}
ggplot(all, aes(x = log.mindrive, y = TTR)) +
  geom_point() +
...
# TO DO : sample rows to get TTR; do subway
