...
# libraries
install.packages(c("googleway","tidyverse","stringr","feather"))
library(googleway)
library(tidyverse)
library(stringr)
library(feather)
...
APIKEY <-  "AIzaSyBooOJs6oAVIWmHlPaN2VKyRjqaWceX79g"
...
for (i in 1:length(all_city_files)) {
  ...
  ids = read_feather(paste0("../place_ids/", all_city_files[i]))
  ...
  write_feather(places_with_reviews, 
                paste0("../reviews/", city_name, "_reviews"))
}
